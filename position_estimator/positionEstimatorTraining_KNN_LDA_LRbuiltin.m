...
for k_it = 1:N_angles+1
    ...
    modelParameters(k_it).n_neighbours = 21;
end
...
for k_it =0:N_angles
    spike_rate_av_trials = make_av_spike_rate(spike_rate, k_it);
    [~, Vs, Ds, ~] = spikes_PCA(spike_rate_av_trials, proportion);
    dir = k_it;
    if k_it == 0
        k_it = N_angles+1;
    end
...
for k_it = 0:N_angles
    ...
    if (k_it ==0) % non-direction specific training
        tbl_x = linearRegression2(principal_spikes_0, velx_tr, k_it, M);
        tbl_y = linearRegression2(principal_spikes_0, vely_tr, k_it, M);
        k_it = N_angles+1;
    else  % direction specific training
        tbl_x = linearRegression2(principal_spikes_tr, velx_tr, k_it, M);
        tbl_y = linearRegression2(principal_spikes_tr, vely_tr, k_it, M);
    end
...
function tbl = linearRegression2(training_input, training_output, dir, pcs)
    ...
    tbl = fitlm(training_input, training_output).Coefficients.Estimate(2:pcs+1);
end
